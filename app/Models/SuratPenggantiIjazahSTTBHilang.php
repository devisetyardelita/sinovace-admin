<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;

class SuratPenggantiIjazahSTTBHilang extends Model
{
    use HasFactory;
            // Menentukan nama tabel secara eksplisit
            protected $table = 'surat_pengganti_ijazah_sttb_hilang';

            // Menentukan field yang bisa diisi secara mass assignment
            protected $fillable = [
                'nama',
                'nik',
                'alamat',
                'no_hp',
                'fotokopi_ijazah_sttb_hilang',
                'fotokopi_akta_kelahiran',
                'surat_keterangan_kehilangan',
                'surat_pernyataan_tanggungjawab',
                'surat_keterangan_saksi',
                'dokumen_kewenangan',
                'lembar_cek_nisn_dapodik',
                'status',
            ];
}

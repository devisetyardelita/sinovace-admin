...
<div class="content">
    <div class="container-xxl flex-grow-1 container-p-y">
      ...
      @endif
      <h3 class="fw-bold mb-3" style="color: white">Data Surat Pengganti Ijazah/STTB Hilang</h3>

          <!-- Search and Filter Container -->
          <div class="d-flex align-items-center">
            <!-- Search -->
            <div class="navbar-nav align-items-left me-2" style="width: 75%;"> <!-- Atur lebar search -->
              <form action="{{ url('surat_pengganti_ijazah_sttb_hilang') }}" method="GET">
          ...
          <!-- /Search and Filter Container -->
            ...
            <div class="divider d-flex justify-content-center align-items-center mt-0">
              <a href="{{ url('surat_pengganti_ijazah_sttb_hilang/excel/') }}" class="btn rounded-pill btn-primary mx-1">Excel</a>
              <a href="{{ url('surat_pengganti_ijazah_sttb_hilang/pdf/') }}" class="btn rounded-pill btn-secondary mx-1">PDF</a>
              <a href="{{ url('surat_pengganti_ijazah_sttb_hilang/create') }}" class="btn rounded-pill btn-primary mx-1">
              ...
              <div class="card">
                <div class="card-body">
                      ...
                      <tbody id="tbody">                         
                        @foreach($surat_pengganti_ijazah_sttb_hilang as $item)
                          <tr>
                            ...
                            <td>{{ $loop->iteration + $surat_pengganti_ijazah_sttb_hilang->firstItem() - 1 }}</td>
                            <td><strong>{{ $item->nama }}</strong></td>
                            <td>{{ $item->nik }}</td>
                            <td>{{ $item->alamat }}</td>
                            <td>{{ $item->no_hp }}</td>
                            <td>
                              @if($item->fotokopi_ijazah_sttb_hilang)
                              <a href="{{ asset($item->fotokopi_ijazah_sttb_hilang) }}" target="_blank">Lihat File</a>
                              @else
                            ...
                            <td>
                                <a href="{{ url('surat_pengganti_ijazah_sttb_hilang/edit/' . $item->id) }}" class="btn btn-secondary btn-sm">
                                  ...
                                  <div class="modal-dialog modal-sm">
                                    <div class="modal-content text-center p-4">
                                      ...
                                      <div class="d-flex justify-content-center gap-3 mb-2">
                                        ...
                                        <form action="{{ url('surat_pengganti_ijazah_sttb_hilang/delete/' . $item->id) }}" method="POST" class="d-inline">
                                            @method('delete')
                                            @csrf
                                ...
                                <a href="{{ url('surat_pengganti_ijazah_sttb_hilang/show/' . $item->id) }}" class="btn btn-dark btn-sm">Detail
                  ...
                  <div class="mt-2">
                    {{ $surat_pengganti_ijazah_sttb_hilang->links() }}
                  </div>
                </div>
              </div>
              <!--/ Bordered Table -->
        ...
        success: function(data) {
          ...
          if (filteredStatuses.length > 0) {
            for (let i = 0; i < filteredStatuses.length; i++) {
              ...
              // Handle fotokopi_ijazah_sttb_hilang display
              var fileSuratLink = filteredStatuses[i]['fotokopi_ijazah_sttb_hilang']
              ? '<a href="/' + filteredStatuses[i]['fotokopi_ijazah_sttb_hilang'] + '" target="_blank">Lihat File</a>'
              : 'File tidak tersedia';
              ...
              html += '<tr>\
                ...
                <td>\
                  <a href="/surat_pengganti_ijazah_sttb_hilang/edit/' + filteredStatuses[i]['id'] + '" class="btn btn-secondary btn-sm">\
                    <i class="bx bx-edit-alt me-1"></i>\
                  </a>\
                  <form action="/surat_pengganti_ijazah_sttb_hilang/delete/' + filteredStatuses[i]['id'] + '" method="POST" class="d-inline">\
                    @method("delete")\
                    @csrf\
                    <button class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah Anda yakin ingin menghapus data ini?\')">\
                      <i class="bx bx-trash me-1"></i>\
                  ...
                  <a href="/surat_pengganti_ijazah_sttb_hilang/show/' + filteredStatuses[i]['id'] + '" class="btn btn-dark btn-sm">Detail\
                    <i class="bx bx-right-arrow-alt"></i>\
                  </a>\
                </td>\
              </tr>';

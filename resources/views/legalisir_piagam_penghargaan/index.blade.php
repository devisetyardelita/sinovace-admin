...
<div class="content">
    <div class="container-xxl flex-grow-1 container-p-y">
      ...
      @endif
      <h3 class="fw-bold mb-3" style="color: white">Data Legalisir Piagam Penghargaan</h3>

          <!-- Search and Filter Container -->
          <div class="d-flex align-items-center">
            <!-- Search -->
            <div class="navbar-nav align-items-left me-2" style="width: 75%;"> <!-- Atur lebar search -->
              <form action="{{ url('legalisir_piagam_penghargaan') }}" method="GET">
          ...
          <!-- /Search and Filter Container -->
            ...
            <div class="divider d-flex justify-content-center align-items-center mt-0">
              <a href="{{ url('legalisir_piagam_penghargaan/export/') }}" class="btn rounded-pill btn-primary mx-1">Excel</a>
              <a href="{{ url('legalisir_piagam_penghargaan/generate_legalisir_piagam_penghargaan_pdf/') }}" class="btn rounded-pill btn-secondary mx-1">PDF</a>
              <a href="{{ url('legalisir_piagam_penghargaan/create') }}" class="btn rounded-pill btn-primary mx-1">
                ...
                <div class="card-body">
                  <div class="table-responsive text-nowrap">
                    <table class="table table-bordered">
                      ...
                      <tbody id="tbody">                         
                        @foreach($legalisir_piagam_penghargaan as $item)
                          <tr>

                            <td>{{ $loop->iteration + $legalisir_piagam_penghargaan->firstItem() - 1 }}</td>
                            <td><strong>{{ $item->nama }}</strong></td>
                            <td>{{ $item->nik }}</td>
                            <td>{{ $item->alamat }}</td>
                            <td>{{ $item->no_hp }}</td>
                            <td>
                              @if($item->piagam_penghargaan)
                              <a href="{{ asset($item->piagam_penghargaan) }}" target="_blank">Lihat File</a>
                              @else
                            ...
                            <td>
                                <a href="{{ url('legalisir_piagam_penghargaan/edit/' . $item->id) }}" class="btn btn-secondary btn-sm">
                                  ...
                                  <div class="modal-dialog modal-sm">
                                    <div class="modal-content text-center p-4">
                                      ...
                                      <div class="d-flex justify-content-center gap-3 mb-2">
                                        ...
                                        <form action="{{ url('legalisir_piagam_penghargaan/delete/' . $item->id) }}" method="POST" class="d-inline">
                                            @method('delete')
                                            @csrf
                                ...
                                <a href="{{ url('legalisir_piagam_penghargaan/show/' . $item->id) }}" class="btn btn-dark btn-sm">Detail
                  ...
                  <div class="mt-2">
                    {{ $legalisir_piagam_penghargaan->links() }}
                  </div>
                </div>
              </div>
              <!--/ Bordered Table -->
        ...
        success: function(data) {
          ...
          if (filteredStatuses.length > 0) {
            for (let i = 0; i < filteredStatuses.length; i++) {
              ...
              // Handle piagam_penghargaan display
              var fileSuratLink = filteredStatuses[i]['piagam_penghargaan']
              ? '<a href="/' + filteredStatuses[i]['piagam_penghargaan'] + '" target="_blank">Lihat File</a>'
              : 'File tidak tersedia';
              ...
              html += '<tr>\
                ...
                <td>\
                  <a href="/legalisir_piagam_penghargaan/edit/' + filteredStatuses[i]['id'] + '" class="btn btn-secondary btn-sm">\
                    <i class="bx bx-edit-alt me-1"></i>\
                  </a>\
                  <form action="/legalisir_piagam_penghargaan/delete/' + filteredStatuses[i]['id'] + '" method="POST" class="d-inline">\
                    @method("delete")\
                    @csrf\
                    <button class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah Anda yakin ingin menghapus data ini?\')">\
                      <i class="bx bx-trash me-1"></i>\
                  ...
                  <a href="/legalisir_piagam_penghargaan/show/' + filteredStatuses[i]['id'] + '" class="btn btn-dark btn-sm">Detail\
                    <i class="bx bx-right-arrow-alt"></i>\
                  </a>\
                </td>\
              </tr>';

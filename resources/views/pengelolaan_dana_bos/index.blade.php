...
<div class="content">
    <div class="container-xxl flex-grow-1 container-p-y">
      ...
      @endif
      <h3 class="fw-bold mb-3" style="color: white">Data Pengelolaan BOS</h3>

          <!-- Search and Filter Container -->
          <div class="d-flex align-items-center">
            <!-- Search -->
            <div class="navbar-nav align-items-left me-2" style="width: 75%;"> <!-- Atur lebar search -->
              <form action="{{ url('pengelolaan_dana_bos') }}" method="GET">
          ...
          <!-- /Search and Filter Container -->
            ...
            <div class="divider d-flex justify-content-center align-items-center mt-0">
              <a href="{{ url('pengelolaan_dana_bos/excel/') }}" class="btn rounded-pill btn-primary mx-1">Excel</a>
              <a href="{{ url('pengelolaan_dana_bos/pdf/') }}" class="btn rounded-pill btn-secondary mx-1">PDF</a>
              <a href="{{ url('pengelolaan_dana_bos/create') }}" class="btn rounded-pill btn-primary mx-1">
                    ...
                    <table class="table table-bordered">
                      <thead>
                        <tr>
                          ...
                          <th>Alamat</th>
                          <th>No. HP/WA</th>
                          <th>Dokumen Pertanggungjawaban Keuangan</th>
                          {{-- <th>Foto KTP</th>
                      ...
                      <tbody id="tbody">                         
                        @foreach($pengelolaan_dana_bos as $item)
                          <tr>

                            <td>{{ $loop->iteration + $pengelolaan_dana_bos->firstItem() - 1 }}</td>
                            <td><strong>{{ $item->nama }}</strong></td>
                            <td>{{ $item->nik }}</td>
                            <td>{{ $item->alamat }}</td>
                            <td>{{ $item->no_hp }}</td>
                            <td>
                              @if($item->dokumen_pertanggungjawaban_keuangan)
                              <a href="{{ asset($item->dokumen_pertanggungjawaban_keuangan) }}" target="_blank">Lihat File</a>
                              @else
                            ...
                            <td>
                                <a href="{{ url('pengelolaan_dana_bos/edit/' . $item->id) }}" class="btn btn-secondary btn-sm">
                                  ...
                                  <div class="modal-dialog modal-sm">
                                    <div class="modal-content text-center p-4">
                                      ...
                                      <div class="d-flex justify-content-center gap-3 mb-2">
                                        ...
                                        <form action="{{ url('pengelolaan_dana_bos/delete/' . $item->id) }}" method="POST" class="d-inline">
                                            @method('delete')
                                            @csrf
                                ...
                                <a href="{{ url('pengelolaan_dana_bos/show/' . $item->id) }}" class="btn btn-dark btn-sm">Detail
        ...
        success: function(data) {
          ...
          if (filteredStatuses.length > 0) {
            for (let i = 0; i < filteredStatuses.length; i++) {
              ...
              // Handle dokumen_pertanggungjawaban_keuangan display
              var fileSuratLink = filteredStatuses[i]['dokumen_pertanggungjawaban_keuangan']
              ? '<a href="/' + filteredStatuses[i]['dokumen_pertanggungjawaban_keuangan'] + '" target="_blank">Lihat File</a>'
              : 'File tidak tersedia';
              ...
              html += '<tr>\
                ...
                <td>\
                  <a href="/pengelolaan_dana_bos/edit/' + filteredStatuses[i]['id'] + '" class="btn btn-secondary btn-sm">\
                    <i class="bx bx-edit-alt me-1"></i>\
                  </a>\
                  <form action="/pengelolaan_dana_bos/delete/' + filteredStatuses[i]['id'] + '" method="POST" class="d-inline">\
                    @method("delete")\
                    @csrf\
                    <button class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah Anda yakin ingin menghapus data ini?\')">\
                      <i class="bx bx-trash me-1"></i>\
                  ...
                  <a href="/pengelolaan_dana_bos/show/' + filteredStatuses[i]['id'] + '" class="btn btn-dark btn-sm">Detail\
                    <i class="bx bx-right-arrow-alt"></i>\
                  </a>\
                </td>\
              </tr>';
